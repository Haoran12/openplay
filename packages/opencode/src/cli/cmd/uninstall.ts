...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall openplay and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g openplay-ai",
      pnpm: "pnpm uninstall -g openplay-ai",
      bun: "bun remove -g openplay-ai",
      yarn: "yarn global remove openplay-ai",
      brew: "brew uninstall openplay",
      choco: "choco uninstall openplay",
      scoop: "scoop uninstall openplay",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "openplay", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using OpenPlay!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# openplay") || content.includes(".openplay/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".openplay/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".openplay/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".openplay"))
    ) {
      continue
    }

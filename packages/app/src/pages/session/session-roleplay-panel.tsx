                        ...
                        {(replay) => (
                            <div class="rounded-lg border border-border-weaker-base bg-background-base p-3 flex flex-col gap-3">
                                <div class="text-12-medium text-text-weak">
                                    {language.t("roleplay.panel.latestEmbodiment")}
                                </div>
                                    ...
                                    <div>
                                        ...
                                        <div class="text-13-regular text-text-base whitespace-pre-wrap flex flex-col gap-1">
                                            <Show when={replay().speech}>
                                                <p>
                                                    {language.t("roleplay.panel.speech")}: {replay().speech}
                                                </p>
                                            </Show>
                                            <Show when={replay().outwardAction}>
                                                <p>
                                                    {language.t("roleplay.panel.outwardAction")}:{" "}
                                                    {replay().outwardAction}
                                                </p>
                                            </Show>
                                            <Show when={replay().actionIntent}>
                                                <p>
                                                    {language.t("roleplay.panel.actionIntent")}: {replay().actionIntent}
                                                </p>
                                            </Show>
                                            <Show when={replay().innerThought}>
                                                <p>
                                                    {language.t("roleplay.panel.innerThought")}: {replay().innerThought}
                                                </p>
                                            </Show>
                                        </div>
                                    </div>
...
export const SessionRoleplayPanel: Component = () => {
    ...
    const [traceMeta] = createResource(sessionID, async (id) => {
    ...
    createResource(childSessionIDs, async (ids) => {
        await Promise.all(
            ids.map(async (id) => {
    ...
    return (
        ...
        >
            <div class="flex-1 min-h-0 overflow-y-auto">
                <div class="p-4 border-b border-border-weaker-base">
                    <div class="flex items-center justify-between gap-3 mb-3">
                        <div class="flex items-center gap-2">
                            <h3 class="text-12-medium text-text-weak uppercase tracking-wider">
                    ...
                    <Show
                        ...
                        fallback={
                            <p class="text-14-regular text-text-weak italic">{language.t("roleplay.panel.noScene")}</p>
                        }
                    >
                        <div class="rounded-lg border border-border-weaker-base bg-surface-base p-3 flex flex-col gap-2">
                            <For each={sceneRows()}>
                                {(row) => (
                                    <div>
                                        <div class="text-11-medium text-text-weak uppercase tracking-wider mb-0.5">
                                            {row.label}
                                        </div>
                                        ...
                                        <div class="rounded-md border border-border-weaker-base bg-background-base p-3">
                                            ...
                                            <Show when={item.presentCharacters.length > 0}>
                                                <div class="mt-1 text-12-regular text-text-weak">
                                                    {language.t("roleplay.panel.presentCharacters")}:{" "}
                                                    {item.presentCharacters.join(", ")}
                                                </div>
                                            </Show>
                                            <Show when={item.impression}>
                                                <div class="mt-1 text-12-regular text-text-weak whitespace-pre-wrap">
                                                    {item.impression}
                                                </div>
                                            </Show>
                                        </div>
                ...
                <div class="p-4">
                    <h3 class="text-12-medium text-text-weak uppercase tracking-wider mb-3">
                            ...
                            <For each={characters()}>
                                {(item) => (
                                    <div class="rounded-lg border border-border-weaker-base overflow-hidden bg-surface-base">
                                        <Show
                                            when={item.agent}
                                            fallback={<RuntimeCharacterCard character={item.runtime} />}
                                        >
                                            {(agent) => (
                                                <CharacterWithRuntimeCard
                                                    ...
                                                    onOpenProfile={
                                                        agent().statePath
                                                            ? () =>
                                                                  openCharacterProfile(agent().statePath, agent().name)
                                                            : undefined
                                                    }
                                                    onEdit={
                                                        agent().statePath
                                                            ? () => openFile(agent().statePath!)
                                                            : undefined
                                                    }
                                                    onOpenDirectory={
                                                        agent().statePath
                                                            ? () => openCharacterDirectory(agent().statePath)
                                                            : undefined
                                                    }
                                                />
                                            )}
